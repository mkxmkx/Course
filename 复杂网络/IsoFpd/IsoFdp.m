...
x=load(netfile);
largex=x;       
nv=size(x,1);    %返回文件中的行数
ND=nv/2;	       
n=max(x(:,2));	 %第二列中的最大值
x(x(:,1)>x(:,2),:)=[];	%文件中第一列值比第二列值大的行


adj_square=sparse(n,n);   %生成全零稀疏矩阵
for i=1:size(x,1)
	mi=x(i,1);
	ni=x(i,2);
	adj_square(mi,ni)=1;
end
adj_square=adj_square+adj_square'+speye(n);  %生成稀疏矩阵完毕

% 10 other mesures tested in the paper:'euclidean','cosine','jaccard'... and so on
if  ~strcmp(metrics,'structure')  %其它衡量指标，非结构相似性
...
if A(1,1) == 1
    A = A-eye(size(A));		%鎶婇偦鎺ユ柟闃典富瀵硅绾垮厓绱犳敼涓?0
end
...
for i = 1:size(label,1)
    if (~isempty(label{i}))&&(label{i}~=0)	%濡傛灉鑺傜偣i涓嶆槸鍣０鐐?
        temp = label{i};	%鏆傛椂鎶婅妭鐐筰鐨勭被鏍囩瀛樺偍涓簍emp
        s(i) = length(temp);	%寰楀埌璇ヨ妭鐐瑰綊灞炵被绨囩殑涓暟
        for j = 1:length(temp)
